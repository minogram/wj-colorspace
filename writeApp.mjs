    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', width: '100vw', height: '100vh', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={mStyles.bottomNav}>
          <button
            style={{ ...mStyles.navBtn, ...(!mobileDrawerOpen ? mStyles.navBtnActive : {}) }}
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        <div style={styles.topBar}>
          ...
          <div style={styles.controls}>
            <ToggleBtn active={autoRotate} onClick={() => setAutoRotate(v => !v)} title="자동 회전">↻</ToggleBtn>
            <ToggleBtn active={showAxes} onClick={() => setShowAxes(v => !v)} title="축 표시">⊞</ToggleBtn>
            <ToggleBtn active={showGrid} onClick={() => setShowGrid(v => !v)} title="그리드 표시">⊟</ToggleBtn>
            <ToggleBtn active={lang === 'en'} onClick={() => setLang(v => v === 'ko' ? 'en' : 'ko')} title="언어 전환">
              {lang === 'ko' ? 'KO' : 'EN'}
            </ToggleBtn>
            {activeHues && (
              <button style={styles.clearBtn} onClick={() => setActiveHues(null)}>하이라이트 해제</button>
            )}
          </div>
          <button style={styles.sidebarToggle} onClick={() => setSidebarOpen(v => !v)} title={sidebarOpen ? '패널 닫기' : '패널 열기'}>
            {sidebarOpen ? '◀' : '▶'}
          </button>
        </div>
...
const styles = {
  ...
  title: { fontSize: 22, fontWeight: 700, color: '#f1f5f9', letterSpacing: '-0.02em', lineHeight: 1.2 },
  subtitle: { fontSize: 12, color: '#475569', marginTop: 4, letterSpacing: '0.02em' },
  controls: { display: 'flex', gap: 6, pointerEvents: 'auto', flexWrap: 'wrap', justifyContent: 'flex-end', maxWidth: 300 },
  iconBtn: {
    width: 36, height: 36, borderRadius: 8, cursor: 'pointer', fontSize: 14,
    display: 'flex', alignItems: 'center', justifyContent: 'center',
  ...
  clearBtn: {
    ...
    color: '#fca5a5', transition: 'all 0.15s ease', whiteSpace: 'nowrap',
  },
  sidebarToggle: {
    width: 32, height: 32, borderRadius: 8, cursor: 'pointer',
    background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)',
    color: '#94a3b8', fontSize: 12, pointerEvents: 'auto', flexShrink: 0,
  ...
  hint: { fontSize: 10, color: '#334155', marginTop: 4, letterSpacing: '0.04em' },
}
...
const mStyles = {
  ...
  topControls: { display: 'flex', gap: 6, pointerEvents: 'auto' },
  bottomNav: {
...
fs.writeFileSync('./colorspace-app/src/App.jsx', content, 'utf8')
console.log('App.jsx written:', content.split('\n').length, 'lines')
